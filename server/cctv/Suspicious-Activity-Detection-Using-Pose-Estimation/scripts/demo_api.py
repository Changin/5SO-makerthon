...
_cfg = "configs/coco/resnet/256x192_res50_lr1e-3_1x.yaml"
_checkpoint = "pretrained_models/fast_res50_256x192.pth"
_save_img = "--save_img"
_vis = "--vis"
_showbox = "--showbox"

args = parser.parse_args(["--cfg", _cfg, "--checkpoint", _checkpoint, "--save_img", _save_img, _vis, _showbox])
...
def sample_video(input_path, output_path, sample_rate=30, is_resized=False):

    print("start sampling...")
    vidcap = cv2.VideoCapture(input_path)
    count = 0
    while(vidcap.isOpended()):
        ret, image = vidcap.read()
        #resize image into 640x360
        if is_resized:
            image = image.resize(640, 360, interpolation=cv2.INTER_AREA)
        if (int(vidcap.get(1)) % 30 == 0):
            print("Saved frame number : %d" % int(vidcap.get(1)))
            cv2.imwrite(output_path + "frame(%d).png" % count, image)

is_loaded = False
def load_model():
    if not is_loaded:
        global model = builder.build_sppe(cfg.MODEL, preset_cfg=cfg.DATA_PRESET)
        model.load_state_dict(torch.load(args.checkpoint, map_location=args.device))
    return model

def pedestrian_detection(input_path, output_path):

    image = read_image(input_path, True)
    ...
    for x, y, w, h in detected:
        cv2.rectangle(image, (x, y), (x + w, y + h), color=(0, 255, 0), thickness=_thickness)

    return image
    
def suspicious_activity_detection(input_path, output_path):

    est = PoseEstimator(args, cfg)
    image = read_image(input_path, True)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    pose = est.process(input_path, image)
    img = est.getImg()
    img = est.vis(img, pose)
    cv2.imwrite(os.path.join(output_path, os.path.basename(input_path)), img)

    result = [pose]
    prediction = est.writeJson(result, outputpath, form=args.format, for_eval=args.eval)

    ret = {}

    ret['is_suspicious'] = 1 if prediction == "Alert" else 0 #0 is unsuspicious, 1 is suspicious
    ret['category'] = 0 if prediction == "Alert" else 1 #0 is climbing, 1 is standing
